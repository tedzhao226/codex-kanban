...
The supported target for this alpha is macOS with Node.js 24 or newer and the Codex Desktop app installed with local tasks. Dated observations on 2026-09-13 recorded Codex Desktop app version `26.908.40834`, build `8881`, and bundled CLI version `0.154.0-alpha.6.2`; the adapter targets private IPC state version 11 and observed follower request versions. These are observations, not compatibility promises. A Desktop update can require adapter changes.
