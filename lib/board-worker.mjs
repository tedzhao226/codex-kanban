...
function syncRuntime(threads) {
  for (const thread of threads) {
    if (['running', 'needs-input', 'error'].includes(thread.runtime) && runtimes.get(thread.id) !== thread.runtime) pendingActivity.add(thread.id);
  }
  const manual = db.prepare('SELECT thread_id FROM card_state WHERE manual_lane IS NOT NULL').all().filter(card => pendingActivity.has(card.thread_id));
