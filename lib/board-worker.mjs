...
function syncRuntime(threads) {
  const resumed = new Set(threads.filter(thread => ['running', 'needs-input', 'error'].includes(thread.runtime) && runtimes.get(thread.id) !== thread.runtime).map(thread => thread.id));
  const manual = db.prepare('SELECT thread_id FROM card_state WHERE manual_lane IS NOT NULL').all().filter(card => resumed.has(card.thread_id));
  if (manual.length) transaction('IMMEDIATE', () => {
    const reset = db.prepare('UPDATE card_state SET manual_lane = NULL, revision = revision + 1 WHERE thread_id = ? AND manual_lane IS NOT NULL');
    let changed = 0;
    for (const card of manual) changed += reset.run(card.thread_id).changes;
    if (changed) db.exec('UPDATE board_meta SET revision = revision + 1 WHERE id = 1');
  });
  runtimes = new Map(threads.map(thread => [thread.id, thread.runtime]));
}
...
function edit(method, { id, column, beforeId, threads, expectedLayoutRevision }) {
  ...
  return transaction('IMMEDIATE', () => {
    ...
    const manualLane = method === 'move' && (state.cards.get(id)?.manual_lane != null || column !== defaultColumn(threads.find(thread => thread.id === id).runtime)) ? column : null;
    db.prepare('INSERT INTO card_state VALUES (?, ?, ?) ON CONFLICT(thread_id) DO UPDATE SET manual_lane = excluded.manual_lane, revision = excluded.revision').run(id, manualLane, revision + 1);
...
parentPort.on('message', ({ id, method, input }) => {
  try {
    ...
    else if (['arrange', 'move', 'reset'].includes(method)) {
      syncRuntime(input.threads);
      result = method === 'arrange' ? transaction('DEFERRED', () => arrange(readState(), input.threads)) : edit(method, input);
    }
