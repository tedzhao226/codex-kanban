...
export async function runCli(argv, { stdout = process.stdout, stderr = process.stderr, stdin = process.stdin, signal } = {}) {
  const write = (stream, text) => stream.write(text.replace(/[\u0000-\u0008\u000b-\u001f\u007f-\u009f]/g, char => '\\u' + char.charCodeAt(0).toString(16).padStart(4, '0')));
  let json = argv.includes('--json'), requestId;
  try {
    if (!argv.length || argv.includes('--help')) { write(stdout, help); return 0; }
    ...
    const emit = (value, readable) => write(stdout, json ? JSON.stringify(clean(value)) + '\n' : readable + '\n');
    ...
    const printView = (view, seen) => {
      ...
      write(stdout, `${oneLine(task.title)} [${oneLine(view.runtime)}]\n`);
      for (const item of view.items) {
        const content = item.kind === 'tool' ? [item.title, item.status ? `(${item.status})` : '', item.detail].filter(Boolean).join('\n') : item.text || '';
        const rendered = `[${oneLine(item.kind || item.role || item.type)}] ${content}${item.images?.length ? `\n[${item.images.length} image(s)]` : ''}`;
        if (!seen || seen.get(item.id) !== rendered) { write(stdout, rendered + '\n'); seen?.set(item.id, rendered); }
      }
    };
