...
function inlineImage(part) {
  if (!['image', 'input_image'].includes(part.type)) return null;
  const image = { name: 'Attached image', dataUrl: part.url ?? part.image_url };
  try { validateImages([image]); return image; }
  catch { return null; }
}
...
function contentText(content) {
  ...
  return content.map(part => {
    ...
    if (/image|audio|file/i.test(part.type ?? '')) return inlineImage(part) ? '' : '[Attachment — open in Codex]';
    return '';
  }).filter(Boolean).join('\n');
}

function userText(text) {
  const reply = /^\s*<send_user_message_question_reply>\s*([\s\S]*?)\s*<\/send_user_message_question_reply>\s*$/.exec(text);
  if (!reply) return text;
  let answers;
  try { answers = JSON.parse(reply[1]); }
  catch { return 'This question reply could not be read. Open it in Codex.'; }
  if (!Array.isArray(answers) || !answers.length || answers.some(answer => typeof answer?.question !== 'string' || typeof answer.answer !== 'string')) {
    return 'This question reply has an unsupported format. Open it in Codex.';
  }
  const plain = value => value.replace(/[\\`*_{}\[\]()<>#+.!|~-]/g, '\\$&');
  return answers.map(({ question, answer }) => `${plain(question).split('\n').map(line => `> ${line}`).join('\n')}\n\n${plain(answer)}`).join('\n\n');
}
...
function normalizeItem(item, turnId, timestamp) {
  ...
  if (['usermessage', 'steeringusermessage', 'agentmessage'].includes(type)) {
    ...
    const images = Array.isArray(content) ? content.map(inlineImage).filter(Boolean) : [];
    const text = item.text ?? contentText(content);
    if (!text && !images.length) return null;
    return { ...base, kind: type === 'agentmessage' ? 'assistant' : 'user', text: type === 'agentmessage' ? text : userText(text),
  ...
  async read(id, path) {
    ...
    function consume(line) {
      ...
      if (p.type === 'user_message' || p.type === 'agent_message') legacy.push({ id: `legacy:${lineNumber}`,
        turnId, timestamp, kind: p.type === 'user_message' ? 'user' : 'assistant', text: p.type === 'user_message' ? userText(p.message ?? '') : p.message ?? '', clientId: null });
    }
    try {
